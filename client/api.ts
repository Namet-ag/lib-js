...
export const CapesApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        getCapeUsers: async (id: string, options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {
            // verify required parameter 'id' is not null or undefined
            assertParamExists('getCapeUsers', 'id', id)
            const localVarPath = `/capes/{id}/users`
                .replace(`{${"id"}}`, encodeURIComponent(String(id)));
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
...
export const CapesApiFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async getCapeUsers(id: string, options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<Array<Profile>>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.getCapeUsers(id, options);
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
            const localVarOperationServerBasePath = operationServerMap['CapesApi.getCapeUsers']?.[localVarOperationServerIndex]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, localVarOperationServerBasePath || basePath);
        },
    }
};
...
export const CapesApiFactory = function (configuration?: Configuration, basePath?: string, axios?: AxiosInstance) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        getCapeUsers(id: string, options?: RawAxiosRequestConfig): AxiosPromise<Array<Profile>> {
            return localVarFp.getCapeUsers(id, options).then((request) => request(axios, basePath));
        },
    };
};
...
export class CapesApi extends BaseAPI {
     ...
     * @memberof CapesApi
     */
    public getCapeUsers(id: string, options?: RawAxiosRequestConfig) {
        return CapesApiFp(this.configuration).getCapeUsers(id, options).then((request) => request(this.axios, this.basePath));
    }
}
